...
function LootMasterML:GetFrame()
	...
	titletext:SetText( string.format("EPGPLootMaster %s by Bushmaster <Steel Alliance> - Darksorrow EU", self:GetVersionString() ) )
...
function LootMasterML:ShowRaidInfoLookup()
    ...
    if not frame then
        ...
        local titletext = titleFrame:CreateFontString(nil,"OVERLAY","GameFontNormalSmall")
        titletext:SetPoint("CENTER",titleFrame,"CENTER",0,1)
        titletext:SetText( string.format("EPGPLootMaster %s by Bushmaster <Steel Alliance> - Darksorrow EU", LootMaster:GetVersionString() ) )
        frame.titleFrame = titleFrame
        --#endregion
