local aName, aObj = ...
if not aObj:isAddonEnabled("FramesResized") then return end

function aObj:FramesResized()
	...
	if FramesResized_SV.RaidInfo_Resize then
		self:moveObject{obj=RaidInfoFrame, x=GetNumGroupMembers() > 0 and 20 or 0, y=17}
...
-->>-- Options panel
	self:addSkinFrame{obj=FramesResizedPanel_LootFrame_Box, ofs=-1}
	self:addSkinFrame{obj=FramesResizedPanel_RaidInfo_Box, ofs=-1}
	self:addSkinFrame{obj=FramesResizedPanel_TraidSkillUI_Box, ofs=-1}
	self:addSkinFrame{obj=FramesResizedPanel_TrainerUI_Box, ofs=-1}
	self:addSkinFrame{obj=FramesResizedPanel_GuildUI_Box, ofs=-1}
